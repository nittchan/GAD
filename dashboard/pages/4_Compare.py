...
<style>
    ...
    .compare-label { color: #8b949e; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; }
    .compare-value { font-family: 'JetBrains Mono', monospace; font-size: 24px; font-weight: 700; }
</style>
...
st.markdown(
    ...
    '<h1 style="font-size:28px;font-weight:700;color:#e6edf3;margin-bottom:8px;">Compare Triggers</h1>'
    '<p style="color:#8b949e;font-size:14px;">Select any two triggers to compare side-by-side.</p>',
    unsafe_allow_html=True,
)
...
for col, trigger, data, result in [(col1, t1, data1, result1), (col2, t2, data2, result2)]:
    with col:
        ...
        <div class="compare-card" style="margin-bottom:16px;">
            <div style="font-size:18px;font-weight:700;color:#e6edf3;margin-bottom:4px;">{trigger.name}</div>
            <div style="color:#8b949e;font-size:12px;margin-bottom:12px;">{trigger.location_label} · {PERIL_LABELS[trigger.peril]}</div>
            <div class="compare-label">Current Value</div>
            <div class="compare-value" style="color:{color}">{value if value is not None else '—'}</div>
            <div style="color:#8b949e;font-size:12px;">{unit}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="compare-card">
            ...
            <div style="font-size:16px;font-weight:600;color:#e6edf3;">{trigger.threshold} {trigger.threshold_unit}</div>
            <div style="color:#8b949e;font-size:12px;margin-top:8px;">Fires when {'above' if trigger.fires_when_above else 'below'} · Status: <span style="color:{color};font-weight:600;">{status_label}</span></div>
        </div>
        """, unsafe_allow_html=True)
